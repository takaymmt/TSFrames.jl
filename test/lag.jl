...
@testset "lag extreme Int values (C2 regression)" begin
    ts = TSFrame(Float64[1.0, 2.0, 3.0], Date(2020,1,1):Day(1):Date(2020,1,3) |> collect)
    # typemin(Int) and typemax(Int) should not crash — return all-missing
    result_min = lag(ts, typemin(Int))
    result_max = lag(ts, typemax(Int))
